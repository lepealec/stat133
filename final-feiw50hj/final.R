...
name <- "Alec Lepe"
github.name <- "lepealec"
email.address <- "lepealec@berkeley.edu"
...
# y[[3]] is 3, 6, 9, ..., 999
num=function(a){
  return(seq(from=a,by=a,to=1000))
}
y <- list(num(1),num(2),num(3),num(4),num(5),num(6),num(7),num(8),num(9),num(10))
...
cw4 <- ChickWeight[sample(seq(1,nrow(ChickWeight)),200),]
...
bdiff <- mean(infants[infants$age>=35,]$wt[is.na(infants[infants$age>=35,]$wt)==FALSE])-mean(infants[infants$age<35,]$wt[is.na(infants[infants$age<35,]$wt)==FALSE])
...
plot(y=mtcars$qsec,x=mtcars$hp,ylab=("qsec"),xlab=("hp"),main="qsec vs. hp")
abline(lm(mtcars$qsec~mtcars$hp)$coefficients[1],lm(mtcars$qsec~mtcars$hp)$coefficients[2])
...
plot(y=mtcars[mtcars$am==1,]$disp,x=mtcars[mtcars$am==1,]$wt,col="Blue",xlab='wt \n lb/1000',ylab='disp (cu. in)',main='disp versus wt')
points(y=mtcars[mtcars$am==0,]$disp,x=mtcars[mtcars$am==0,]$wt,col="Red")
...
plot(y=WorldBank$life.expectancy, x=WorldBank$fertility.rate,col=WorldBank$region,type="p",cex=0.2,xlab=('Fertility Rate'),ylab='Life Expectancy',main='
       Life Expectancy vs Fertility Rate')
abline(v=2.1)
par(mfrow=c(1,2))
plot(y=WorldBank[WorldBank$year==1960,]$life.expectancy, x=WorldBank[WorldBank$year==1960,]$fertility.rate,col=WorldBank[WorldBank$year==1960,]$region,type="p",cex=0.2,xlab=('Fertility Rate'),ylab='Life Expectancy',main='
       Life Expectancy vs Fertility Rate')
plot(y=WorldBank[WorldBank$year==2013,]$life.expectancy, x=WorldBank[WorldBank$year==2013,]$fertility.rate,col=WorldBank[WorldBank$year==2013,]$region,type="p",cex=0.2,xlab=('Fertility Rate'),ylab='Life Expectancy',main='
       Life Expectancy vs Fertility Rate')
...
hist(WorldBank[WorldBank$lending=='IDA',]$GDP[is.na(WorldBank[WorldBank$lending=='IDA',]$GDP)==FALSE],xlab="GDP",main="Histogram of GDP \n of IDA Rated Countries")
...
PermDiff <- function(cases, controls, k=5000){
  vec=c(cases,controls)
  output=c()
  for (i in seq(1,k)){
    pull=sample(vec,length(cases))
    output[i]=mean(pull)-mean(vec)
  }
  return(output)
}
...
# There are [k] players that buy a ticket. #3 winning numbers
...
NumJackpot <- function(k, B){
  win=sort(sample(seq(1,19),3))
  n.jackpot=c()
  for (i in seq(1,B)){
    winners=0
    for (j in seq(1,k)){
      player=sort(sample(seq(1,19),3))
      if (sum(player)==sum(win) & sd(player)==sd(win)){
        winners=winners+1
      }
    }
    n.jackpot[i]=winners
...
par(mfrow=c(2,2))
hist(NumJackpot(10000,1000))
hist(NumJackpot(50000,1000))
hist(NumJackpot(100000,1000))
hist(NumJackpot(500000,1000))
#################################################################
...
dna2=dna[unlist(lapply(strsplit(dna,""),length))%%3!=0]
