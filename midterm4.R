...
text1=c()
for (i in seq(1,length(phrases))){
  word=unlist(strsplit(phrases[i],""))
  if (word[length(word)-1]=='o'){
    text1[length(text1)+1]=paste(word,collapse="")
  }
}
...
for (i in seq(1,length(phrases))){
  word=unlist(strsplit(phrases[i],""))
  if (word[1]=='d'){
    text2[length(text2)+1]=paste(word,collapse="")
  }
}
...
even <- paste("even",seq(from=2,by=2,len=1000),sep="")
...
abbreviate <- function(vector, k){
  ...
  for (i in seq(1,length(vector))){
    if (k>=length(unlist(strsplit(vector[i],"")))){
      output[i]=vector[i]
    } else {
      output[i]=paste(as.vector(unlist(strsplit(vector[i],""))[seq(1,k)]),collapse="")
    }
  }
  return(output)
}