...
# Input variable :
# <x> : a numeric vector of length n #length of vector is n
# <k> : an integer

# Output variable
# <x.powers> : A matrix of size [n x k] or [length(x),k] where the first column is x, the second column x^2, the third column x^4, etc.
#              the column names should be : "x", "x^2", "x^3" etc.

powers <- function(x, k){
    power_matrix=matrix(nrow=length(x),ncol=k)  ; i=1
    while (i<=k){
      power_matrix[,i]=x**i
      i=i+1
    }
    if (k==1){
      dimnames(power_matrix)=list(seq(1,length(x)),"x")
    }else{
      dimnames(power_matrix)=list(seq(1,length(x)),c("x",paste("x^",seq(2,k),sep="")))
    }
    x.powers=power_matrix
    return (x.powers)
}
...
bootstrapVarEst <- function(x, B){
  i=1 ; mu_i=seq(1,B)
  while (i<=B){
    mu_i[i]=mean(sample(x,replace=TRUE))
    i=i+1
  }
...
jackknifeVarEst =function(x){
  n=length(x)       ;     i=1
  total_sum=sum(x)  ;     mu_i=c(seq(1,n))
  while (i<=n){
    mu_i[i]=(total_sum-x[i])/(n-1)
    i=i+1
  }
  ...
  if (type=="bootstrap"){
    return(bootstrapVarEst(x,B))
      ...
      return (jackknifeVarEst(x))
        ...
        stop("type not 'bootstrap' or 'jacnkniffe'")
      }
  }